...
function parseChatMarkdown(text: string, modelSlug: string): AgenticChatLog {
  ...
  for (const block of blocks) {
    ...
    const asstMatch = trimmed.match(/^## Assistant\s*\((.*?)\)(?:\s*\n([\s\S]*))?$/)
    if (!asstMatch) continue

    const headerParts = asstMatch[1].split('·').map(s => s.trim())
    const modelName = headerParts[0] ?? ''
    const duration = headerParts.length > 1 ? parseDuration(headerParts[1]) : ''

    let body = asstMatch[2] ? asstMatch[2].trim() : ''

    let thinking = ''
    const thinkMatch = body.match(/_Thinking:_\n\n([\s\S]*?)(?=\n\n\*\*Tool:|$)/)
    if (thinkMatch) {
    ...
    let content = (firstTool >= 0 ? body.slice(0, firstTool) : body)
      .replace(/<\|channel>[\s\S]*?<channel\|>/g, '')
      .replace(/\d+-hour timer:.*/g, '')
      .trim()
      .replace(/\n{3,}/g, '\n\n')

    messages.push({ role: 'assistant', modelName, duration, content, toolCalls, thinking })
  }
...
export const AGENTIC_MODELS: AgenticModel[] = [
  { slug: 'gpt-oss-20b-medium', name: 'GPT OSS 20B', vendor: 'OpenAI', params: '20B A3.6B (MoE)', iconifyId: 'simple-icons:openai' },
  { slug: 'gemma-4-e4b', name: 'Gemma 4 E4B', vendor: 'Google', params: '4B active (MoE)', iconifyId: 'simple-icons:googlegemini' },
  { slug: 'gemma-4-19b-reap', name: 'Gemma 4 19B', vendor: 'Google', params: '19B A4B (REAP-pruned)', iconifyId: 'simple-icons:googlegemini' },
  { slug: 'glm-4-7-flash-23b-reap', name: 'GLM 4.7 Flash 23B', vendor: 'Zhipu AI', params: '23B A3B (REAP-pruned)', iconifyId: 'logos:zhipuai' },
  { slug: 'cerebras-qwen3-25b-reap', name: 'Qwen3 Coder 25B', vendor: 'Cerebras (Qwen)', params: '25B A3B (REAP-pruned)', iconifyId: 'simple-icons:qwen' },
  { slug: 'sushi-coder-9b', name: 'Sushi Coder', vendor: 'Alibaba (community)', params: '9B', iconifyId: 'simple-icons:qwen', thinkingLevel: 'low' },
  { slug: 'devstral-small-2-2512', name: 'Devstral Small 2', vendor: 'Mistral AI', params: '24B', iconifyId: 'simple-icons:mistral' },
] as const
